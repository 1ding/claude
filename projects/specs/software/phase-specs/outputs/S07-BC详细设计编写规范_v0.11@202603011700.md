...
| 第2章    | BC上下文概述         | 必选      | 设计单元定位、版本范围、业务边界、场景索引；架构约束、并发幂等策略、安全约束；文档阅读的背景上下文 |
...
| 第9章    | 后端工程结构设计     | 按需      | 模块划分、包结构、核心类清单；小BC或BC总体设计已统一定义时可省略                                   |
| 第10章   | 权限设计             | 必选      | 权限点定义、角色权限矩阵、API权限映射、数据权限规则、前端权限控制                                  |
| 第11章   | 单元测试设计         | 必选      | 测试范围与策略、关键测试用例（Given-When-Then格式）、测试数据准备方案                              |
| 第12章   | Task细化与工作量估算 | 必选      | Task清单、Task依赖关系表、开发顺序建议；实现计划的最终交付物                                       |
| 附录A    | 专属错误码定义       | 按需      | 本设计单元新增的专属错误码清单，按参数/冲突/业务/基础设施错误分类                                  |
| 附录B    | 缓存与性能设计明细   | 按需      | 缓存项清单、性能优化措施；有性能/缓存设计需求时启用                                                |
| 附录C    | 术语表               | 推荐      | 本设计单元核心术语；与BC总体设计重复的术语标注引用来源即可                                         |
...
| 第9章            | 小BC，或BC总体设计已统一定义工程结构   | 可省略；在BC总体设计中统一说明，各设计单元不重复               |
| 第6章（RPC部分） | 非微服务架构                           | 可省略RPC接口设计小节，仅保留API和消息接口部分                 |
| 各章详细内容     | 未来实施版本（P2、P3等非当前展开版本） | 各章仅保留概述表和清单表，不展开详细设计                       |
| 附录A            | 本设计单元无专属错误码                 | 可省略                                                         |
| 附录B            | 无缓存或性能优化设计需求               | 可省略                                                         |
| 附录C            | 与BC总体设计术语表完全重叠             | 可省略，直接引用BC总体设计术语表                               |
...
本章规定BC详细设计文档正文各章（第1章至第12章）的编写规范，包括各章的输出结构、子节定义、粒度标准、约束与规则及典型示例。
...
| 小节               | 输出形式   | 说明                                         |
| ------------------ | ---------- | -------------------------------------------- |
| 2.1 设计单元定位   | 定义表     | 键值对表格，每行一个定位属性                 |
| 2.2 版本范围       | 版本清单表 | 每行一个版本，标注核心交付内容和状态         |
| 2.3 业务边界       | 双清单表   | "范围内"和"范围外"各一张表                   |
| 2.4 场景索引       | 索引表     | 引用需求文档中的场景定义，不重复展开场景内容 |
| 2.5 架构约束引用   | 约束引用表 | 引用BC总体设计中的架构决策，不重复定义       |
| 2.6 并发与幂等策略 | 策略定义表 | 统一方案，各接口不再逐一重复                 |
| 2.7 安全约束       | 安全约束表 | 引用BC总体设计安全设计，补充本设计单元细节   |
...
-   如BC总体设计已给出完整的状态机定义，第4章引用即可；如仅给出概要，第4章应补充完整的转换规则表和状态机图
-   并发与幂等策略应给出统一方案，后续第6章接口设计中不再逐一重复说明，仅在需要偏离统一方案时标注差异
...
本章由两部分组成：流程总览（3.1节）和各流程详细设计（3.2节起，每个流程一个小节）。
...
| 三级流程 | 单接口内部处理步骤                   | 步骤表        | 不展开（在第6章接口设计中体现） |
...
-   步骤表中的"业务规则引用"列应引用规则编号，不得重复规则内容——规则的完整定义在第4章领域模型设计中
...
流程设计的推导顺序：
1.   从第2章场景索引表出发，为每个场景建立场景→流程的映射，填写流程清单表
2.   确定每个流程的层级（一级/二级），选择对应的图示工具
3.   绘制流程图，识别参与者和关键步骤
4.   细化步骤表，在"业务规则引用"列中引用第4章将定义的规则编号
...
| 10.3 API权限映射  | API权限汇总表             | 每个API对应的权限标识，与第6章交叉引用 |
...
-   每个错误码应可追溯到第4章业务规则表或第6章接口设计中的关联错误码字段
...
错误码定义的推导顺序：
1.   从第4章业务规则表出发，每条规则的"违反后果"列中引用的错误码需要在此定义
2.   从第6章接口设计出发，每个API契约概述表的"关联错误码"列中引用的错误码需要在此定义
3.   检查是否有遗漏——第4章状态机定义中的非法转换、第3章异常处理表中的错误场景，也可能需要专属错误码
