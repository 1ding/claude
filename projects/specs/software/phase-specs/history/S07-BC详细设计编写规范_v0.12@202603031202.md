...
| 第3章    | 业务流程设计         | 必选      | 端到端业务流程图、步骤表、异常处理表；揭示跨组件协作时序和异常处理策略                             |
...
第3章由流程总览和各流程详细定义两部分组成，结构如下。

| 小节                  | 说明                                                             |
| --------------------- | ---------------------------------------------------------------- |
| 3.1 流程总览          | 列出本BC所有流程的编号、名称、触发场景等                         |
| 3.2 {流程名称}        | 顶层流程或子流程的详细设计，每个流程独占一节，按编号顺序排列     |
...
流程清单从第2章场景索引推导：为每个场景建立场景→流程的映射，确定每个流程是顶层流程还是子流程，填写流程总览表。
...
流程图使用ASCII图绘制，遵循《C02-格式规范》的图块规范。图示工具视目的选择（详见§3.3.4）。未来版本的流程仅保留概述和总图，不展开流程步骤和异常处理。
