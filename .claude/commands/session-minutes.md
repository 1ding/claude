...
-   恢复上次中断的**文档编写/修订任务** → `/doc-resume`（从交接文件恢复，更精确）
...
创建纪要文件，保存到 `sessions/` 目录（如不存在则创建）；写入后立即输出 `[CAP DELTA]` 标记（见容量管理规范）。

文件命名：`纪要_{YYYYMMDDHHmm}.md`（或用户指定路径）
...
-   如当前会话有交接文件（`plans/交接_*.md`），新会话应优先使用 `/doc-resume` 恢复，纪要作为补充上下文
