...
```
projects/{项目}/
├── drafts/                    # 工作区（仅最新常规版 + 其上的R版）
│   ├── 01-战略规划_v0.11@202602221200.md      ← 最新常规版本
│   ├── 01-战略规划_v0.11_R01@202602231000.md  ← 基于v0.11的修订
│   └── 01-战略规划_v0.11_R02@202602241500.md
├── outputs/                   # 正式版本区（仅最新正式版）
│   ├── 01-战略规划_v0.11@202602221200.md  ← 当前正式版
│   └── ...
└── history/                   # 历史版本归档（旧常规版 + R版）
...
+   **操作步骤**：
    ...
    4.  将 drafts/ 中所有**旧常规版本**（非最新）移动到 history/
    5.  将 drafts/ 中所有**R 版本文件**移动到 history/
    6.  在修订记录中标注该版本为正式版本
+   **drafts/ 保留原则**：晋升后仅保留最新常规版 + 其上的 R 版；旧常规版和 R 版均归入 history/
...
**行宽约束**：修订记录表的每一行，显示宽度不得超过 200。
计算规则：中文字符及中文符号各占 2 个显示宽度；英文字符及英文符号各占 1 个显示宽度。
当 `修订内容` 内容较长时，应精简描述或拆分为多行（每行对应独立的修订摘要）。
