...
+   本附录规定各标注类型的 Markdown 输入与 HTML 输出对照，执行时须按本附录示例选用对应 CSS 组件。
...
+   本附录规定从 Markdown 输入到 HTML 输出的完整转换示例，覆盖标题页、章节页、两栏内容页、单栏内容页四种页面类型，执行时须以本附录为基准校验输出结构。
...
+   本附录为生成 HTML 后的强制逐项自检清单，交付前必须全部核对通过，不得跳过任何检查项。
...
+   本附录规定从 Markdown 到 HTML 的标准转换工作流程，执行每次转换任务时须遵照本附录流程。
...
+   本附录为简化验证清单，覆盖转换质量的关键检查点；完整清单见附录 C，两者须同时执行。
...
+   本附录规定执行时最容易出错的强制禁止事项和强制执行事项，违反任何一项即视为不合格输出。
...
| R.03   | 2026-03-07 | 附录 A-F 全部由资料性改为规范性；更新各附录引导语为强制执行语气                                                                                          | AI     |
