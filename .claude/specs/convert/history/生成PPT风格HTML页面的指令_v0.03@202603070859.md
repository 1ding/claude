...
| v0.03  | 2026-03-07 | 内容回滚：恢复被删除的标注处理示例（附录A）、完整转换示例（附录B）、完整质量检查清单（附录C）；补充 2.4 节引用；格式保持 C11/C01/C02 规范；对齐 refs：6.3 补充步骤 3 元素清单示例和步骤 4 精确计算表；6.4 改为 ❌/✓ 紧凑化技巧示例；新增附录 D（工作流）、附录 E（验证清单）、附录 F（关键提醒） | AI     |
