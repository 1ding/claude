...
+   根据页面内容特征，按以下优先级选择布局。`[LOGIC:*]` 标注是布局选择的首要依据。

+   **基于逻辑结构选择**（优先级 0，最高优先级）：

+   当页面包含 `[LOGIC:*]` 标注时，按逻辑类型确定页面整体布局：

    | `[LOGIC:*]` 类型     | 页面整体布局                                                      |
    | -------------------- | ----------------------------------------------------------------- |
    | `cause-effect`       | 上下分区：上区原因（`.card-key`/`.highlight-row`）→ 下区结果列表  |
    | `compare`            | 左右两栏（`.two-column-wrapper`），或 `.compare-container`        |
    | `progressive`        | `.flow-container` 流程链，或 `.timeline-bar` + 说明列表           |
    | `classify`           | `.grid-4` 网格卡片（每组一卡），或分组标题 + 分组列表             |
    | `hierarchy`          | 上下分区：上区 `.card-key` 核心要点 → 下区 `.list-items` 支撑项   |
    | `total-parts`        | 上区 `.highlight-row` 总述 → 下区列表或网格                       |
    | `condition`          | 上下分区：上区条件/触发 → 下区响应措施；或 `.flow-container` 条件链 |
    | `evolution`          | `.timeline-bar` 时间线 + 阶段说明                                  |
    | `matrix`             | `.grid-table` 网格表格，行列交叉                                   |
    | `parallel`           | `.grid-4` 网格卡片（非纯列表）                                     |

+   **多层结构布局组合**：当标注为 `[LOGIC:L1→L2]` 时，L1 决定页面级布局，L2 决定区块内布局：

-   L1 为 `cause-effect` → 页面分为上下两区；L2 决定每个区内部的组织方式
-   L1 为 `classify` → 页面分为 2-4 个并列区块（网格/栏）；L2 决定每个区块内的排列
-   L1 为 `hierarchy` → 页面分为上（核心）下（支撑）；L2 决定下区的具体布局
-   L1 为 `compare` → 页面分为左右两栏；L2 决定每栏内部的结构

+   **基于标注类型选择**（优先级 1，当无 `[LOGIC:*]` 时）：

-   有 `[CODE:double]` → 两栏布局（`.two-column-wrapper`）
-   有 `[CHART:compare]` 或成对 `[DATA]` → 两栏布局
-   有 4-6 个 `[DATA]` → 2×2 或 3×2 网格布局（`.data-grid`）
-   要点数 ≥6 且字数少 → 3 列网格布局

+   **基于空间利用选择**（优先级 2，见 §5.7）：

-   内容高度 <175px → 强制升级为 `.highlight-row` / `.card` / `.grid-4`
-   列表项 ≤5 条且 <200px → 使用 `.list-items.spacious`（间距扩展）
-   连续 ≥3 页纯列表 → 第 3 页起切换为非列表布局

+   **默认回退**（优先级 3）：

-   其他情况 → `.content-box` + `.list-items`（但应极少触发此回退，因多数内容可识别出逻辑结构）
...
+   内容区可用高度为 349px，应充分利用，避免大面积空白。以下规则确保页面视觉填充率 ≥70%（内容实际占用 ≥245px）。

-   **列表间距自适应**：当列表项 ≤5 条且总内容高度 <200px 时，自动增加列表项间距（`margin-bottom` 从 10px 提升至 16-20px），使列表自然扩展填充空间。
-   **短内容升级策略**：当内容高度 <50% 可用空间（<175px）时，应采取以下至少一项措施：
    -   使用 `.highlight-row` 替代 `.list-items`（增加 padding 和视觉重量，每条约 40-45px）
    -   使用 `.card-key` 或 `.card` 包裹核心要点（增加边框和 padding）
    -   使用 `.grid-4`（2×2 网格）将要点转为独立卡片
    -   使用上下分区：上区放 `.card-key` 摘要或 `.flow-container`，下区放列表
-   **数据页扩展**：当页面包含 `[DATA]` 标注时，应优先使用 `.data-card` + `.data-grid` 组件，数据卡片自然占用更多空间（每卡约 70-90px）。
-   **字号弹性**：当页面内容少于 250 字且无代码/图表时，正文字号可从 13px 提升至 14px，行高从 1.6 提升至 1.8，以提升可读性并填充空间。

## 5.8 布局多样性规则

+   HTML 生成时应主动避免连续页面使用相同布局，提升视觉节奏感。

-   **连续列表检测**：生成过程中追踪连续纯列表页面数量。当连续 ≥3 页为纯 `.list-items` 布局时，第 3 页起应自动升级为以下布局之一：
    -   `.highlight-row` 格式（高亮背景条）
    -   上下分区（`.card-key` + `.list-items`）
    -   `.grid-4` 网格卡片
-   **布局轮换建议**：相邻内容页应尽量使用不同的视觉形式，推荐的轮换序列为：列表页 → 卡片/网格页 → 列表页 → 流程/时间线页 → 两栏/对比页。
...
## 8.7 辅助样式
    ...
    /* 间距扩展 — 短列表自动填充空间 */
    .list-items.spacious li { margin-bottom: 16px; }
    .list-items.expanded li { margin-bottom: 20px; font-size: 14px; line-height: 1.8; }
